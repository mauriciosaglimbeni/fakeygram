<?php
    ...
    // UPDATING THE DB
    $updateFriend = "UPDATE friendship SET fStatus = 'F 'WHERE fromWho ='$friend_id' AND toWho = '$user_id'"; 
    $updateFriendStatus = mysqli_query($conn,$updateFriend) or die(mysqli_error($conn));

    if($updateFriendStatus){
        header('Location: ../friends.php?message= Friend request accepted!');
    }else{
        header('Location: ../friends.php?message= There was an error accepting your request');
    }
?>